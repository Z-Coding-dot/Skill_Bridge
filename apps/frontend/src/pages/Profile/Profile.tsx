...
export const Profile = () => {
  ...
  const { data = [], isPending: isTasksPending } = useQuery<Task[]>({
    queryKey: ["my-tasks"],
    queryFn: getMyTasks,
  });
        ...
        <div className="mt-16 flex flex-col sm:flex-row justify-center gap-4 mb-16">
          <div className="sm:w-1/3 flex flex-col items-center gap-4">
            <div className="rounded-lg bg-card-bg p-3 sm:p-5 w-full flex flex-col items-center gap-4">
              ...
              <p>{profile.email}</p>
              <Link
                to={`/${ROUTES.DASHBOARD}/${ROUTES.SETTINGS}`}
                className="flex items-center gap-3 w-full justify-center text-white hover:text-gray-400"
              >
            ...
            <div className="bg-card-bg rounded-lg w-full">
              ...
              <div className="flex items-center p-2 sm:p-4 gap-4 flex-wrap">
                {profile.skills.map((skill) => (
                  <p
                    key={skill.id}
                    className="px-3 py-1 rounded-full bg-2card text-trinary"
                  >
                    {skill.label}
                  </p>
                ))}
          ...
          <div className="sm:w-2/3 flex flex-col gap-4">
            ...
            <div className="w-full p-6 rounded-lg bg-card-bg">
              ...
              {isTasksPending ? (
                <p className="text-center mb-5 text-trinary text-sm">
                  Loading tasks...
                </p>
              ) : data.length === 0 ? (
                ...
                <ul>
                  {data.map((task) => (
                    <li key={task.id} className="mb-4 text-sm text-stone-300">
                      {task.title}
                    </li>
                  ))}
                </ul>
