...
var Geometry = class {
	...
	computeBoundingBox(attr) {
		...
		const scale$6 = this.bounds.scale;
		min.set(Infinity);
		max.set(-Infinity);
...
var arrayCacheF32 = {};
var Program = class {
	constructor(gl, { vertex: vertex$8, fragment: fragment$7, uniforms = {}, transparent = false, cullFace = gl.BACK, frontFace = gl.CCW, depthTest = true, depthWrite = true, depthFunc = gl.LEQUAL } = {}) {
		if (!gl.canvas) console.error("gl not passed as first argument to Program");
		this.gl = gl;
		this.uniforms = uniforms;
		this.id = ID$3++;
		if (!vertex$8) console.warn("vertex shader not supplied");
		if (!fragment$7) console.warn("fragment shader not supplied");
...
function flatten(a) {
	...
	const length$4 = arrayLen * valueLen;
	let value = arrayCacheF32[length$4];
	if (!value) arrayCacheF32[length$4] = value = new Float32Array(length$4);
...
var Renderer = class {
	...
	enable(id$2) {
		if (this.state[id$2] === true) return;
		this.gl.enable(id$2);
		this.state[id$2] = true;
	}
	disable(id$2) {
		if (this.state[id$2] === false) return;
		this.gl.disable(id$2);
		this.state[id$2] = false;
	}
	setBlendFunc(src, dst, srcAlpha, dstAlpha) {
...
function invert$2(out, a) {
	...
	let dot$4 = a0 * a0 + a1 * a1 + a2 * a2 + a3 * a3;
	let invDot = dot$4 ? 1 / dot$4 : 0;
	out[0] = -a0 * invDot;
	out[1] = -a1 * invDot;
	out[2] = -a2 * invDot;
	out[3] = a3 * invDot;
...
var Mat4 = class extends Array {
	...
	compose(q, pos, scale$6) {
		compose(this, q, pos, scale$6);
		return this;
	}
	decompose(q, pos, scale$6) {
		decompose(this, q, pos, scale$6);
		return this;
	}
	getRotation(q) {
		getRotation(q, this);
...
var Transform = class {
	...
	lookAt(target, invert$3 = false) {
		if (invert$3) this.matrix.lookAt(this.position, target, this.up);
...
var Camera = class extends Transform {
	...
	frustumIntersectsSphere(center, radius) {
		const normal$1 = tempVec3b$1;
		for (let i = 0; i < 6; i++) {
			const plane = this.frustum[i];
			if (normal$1.copy(plane).dot(center) + plane.constant < -radius) return false;
		}
		return true;
	}
};
...
var ID = 1;
var Texture = class {
	constructor(gl, { image, target = gl.TEXTURE_2D, type = gl.UNSIGNED_BYTE, format = gl.RGBA, internalFormat = format, wrapS = gl.CLAMP_TO_EDGE, wrapT = gl.CLAMP_TO_EDGE, wrapR = gl.CLAMP_TO_EDGE, generateMipmaps = target === (gl.TEXTURE_2D || gl.TEXTURE_CUBE_MAP), minFilter = generateMipmaps ? gl.NEAREST_MIPMAP_LINEAR : gl.LINEAR, magFilter = gl.LINEAR, premultiplyAlpha = false, unpackAlignment = 4, flipY = target == (gl.TEXTURE_2D || gl.TEXTURE_3D) ? true : false, anisotropy = 0, level = 0, width, height = width, length: length$4 = 1 } = {}) {
		this.gl = gl;
		this.id = ID++;
		this.image = image;
		this.target = target;
...
var Plane = class Plane extends Geometry {
	constructor(gl, { width = 1, height = 1, widthSegments = 1, heightSegments = 1, attributes = {} } = {}) {
		...
		const normal$1 = new Float32Array(num * 3);
		const uv$1 = new Float32Array(num * 2);
		const index = numIndices > 65536 ? new Uint32Array(numIndices) : new Uint16Array(numIndices);
		Plane.buildPlane(position, normal$1, uv$1, index, width, height, 0, wSegs, hSegs);
	...
	static buildPlane(position, normal$1, uv$1, index, width, height, depth, wSegs, hSegs, u = 0, v = 1, w = 2, uDir = 1, vDir = -1, i = 0, ii = 0) {
		const io = i;
		...
		for (let iy = 0; iy <= hSegs; iy++) {
			...
			for (let ix = 0; ix <= wSegs; ix++, i++) {
				...
				normal$1[i * 3 + u] = 0;
				normal$1[i * 3 + v] = 0;
				normal$1[i * 3 + w] = depth >= 0 ? 1 : -1;
				uv$1[i * 2] = ix / wSegs;
				uv$1[i * 2 + 1] = 1 - iy / hSegs;
...
var Box = class extends Geometry {
	constructor(gl, { width = 1, height = 1, depth = 1, widthSegments = 1, heightSegments = 1, depthSegments = 1, attributes = {} } = {}) {
		...
		let i = 0;
		let ii = 0;
		Plane.buildPlane(position, normal$1, uv$1, index, depth, height, width, dSegs, hSegs, 2, 1, 0, -1, -1, i, ii);
		i += (dSegs + 1) * (hSegs + 1);
		ii += dSegs * hSegs;
		Plane.buildPlane(position, normal$1, uv$1, index, depth, height, -width, dSegs, hSegs, 2, 1, 0, 1, -1, i, ii);
		i += (dSegs + 1) * (hSegs + 1);
		ii += dSegs * hSegs;
		Plane.buildPlane(position, normal$1, uv$1, index, width, depth, height, dSegs, wSegs, 0, 2, 1, 1, 1, i, ii);
		i += (wSegs + 1) * (dSegs + 1);
		ii += wSegs * dSegs;
		Plane.buildPlane(position, normal$1, uv$1, index, width, depth, -height, dSegs, wSegs, 0, 2, 1, 1, -1, i, ii);
		i += (wSegs + 1) * (dSegs + 1);
		ii += wSegs * dSegs;
		Plane.buildPlane(position, normal$1, uv$1, index, width, height, -depth, wSegs, hSegs, 0, 1, 2, -1, -1, i, ii);
		i += (wSegs + 1) * (hSegs + 1);
		ii += wSegs * hSegs;
		Plane.buildPlane(position, normal$1, uv$1, index, width, height, depth, wSegs, hSegs, 0, 1, 2, 1, -1, i, ii);
		Object.assign(attributes, {
	...
	constructor(gl, { radiusTop = .5, radiusBottom = .5, height = 1, radialSegments = 8, heightSegments = 1, openEnded = false, thetaStart = 0, thetaLength = Math.PI * 2, attributes = {} } = {}) {
		...
		function addCap(isTop) {
			...
			for (x = 0; x <= rSegs; x++) {
				...
				uv$1.set([cosTheta * .5 + .5, sinTheta * .5 * sign + .5], i * 2);
				i++;
			}
			for (x = 0; x < rSegs; x++) {
...
var Torus = class extends Geometry {
	constructor(gl, { radius = .5, tube = .2, radialSegments = 8, tubularSegments = 6, arc = Math.PI * 2, attributes = {} } = {}) {
		...
		const vertex$8 = new Vec3();
		const normal$1 = new Vec3();
		let idx = 0;
		for (let j = 0; j <= radialSegments; j++) for (let i = 0; i <= tubularSegments; i++, idx++) {
			const u = i / tubularSegments * arc;
			const v = j / radialSegments * Math.PI * 2;
			vertex$8.x = (radius + tube * Math.cos(v)) * Math.cos(u);
			vertex$8.y = (radius + tube * Math.cos(v)) * Math.sin(u);
			vertex$8.z = tube * Math.sin(v);
			vertices.set([
				vertex$8.x,
				vertex$8.y,
				vertex$8.z
			...
			normal$1.sub(vertex$8, center).normalize();
			normals.set([
				normal$1.x,
				normal$1.y,
				normal$1.z
...
function Orbit(object, { element = document, enabled = true, target = new Vec3(), ease = .25, inertia = .85, enableRotate = true, rotateSpeed = .1, autoRotate = false, autoRotateSpeed = 1, enableZoom = true, zoomSpeed = 1, zoomStyle = "dolly", enablePan = true, panSpeed = .1, minPolarAngle = 0, maxPolarAngle = Math.PI, minAzimuthAngle = -Infinity, maxAzimuthAngle = Infinity, minDistance = 0, maxDistance = Infinity } = {}) {
	...
	function panLeft(distance$2, m) {
		tempVec3$4.set(m[0], m[1], m[2]);
		tempVec3$4.multiply(-distance$2);
		panDelta.add(tempVec3$4);
	}
	function panUp(distance$2, m) {
	...
	function handleAutoRotate() {
		const angle$1 = 2 * Math.PI / 60 / 60 * autoRotateSpeed;
		sphericalDelta.theta -= angle$1;
	}
	...
	function handleTouchStartDollyPan(e) {
		if (enableZoom) {
			...
			let distance$2 = Math.sqrt(dx * dx + dy * dy);
			dollyStart.set(0, distance$2);
		}
		if (enablePan) {
...
var Raycast = class {
	...
	intersectMeshes(meshes, { cullFace = true, maxDistance, includeUV = true, includeNormal = true, output = [] } = {}) {
		...
		for (let i = hits.length - 1; i >= 0; i--) {
			...
			for (let j = start; j < end; j += 3) {
				...
				const distance$2 = this.intersectTriangle(a, b, c, cullFace, origin, direction, faceNormal);
				if (!distance$2) continue;
				if (maxDistance && distance$2 > localMaxDistance) continue;
				if (!localDistance || distance$2 < localDistance) {
					localDistance = distance$2;
					closestA = ai;
					closestB = bi;
					closestC = ci;
		...
		hits.sort((a$1, b$1) => a$1.hit.distance - b$1.hit.distance);
		return hits;
	}
	...
	intersectTriangle(a, b, c, backfaceCulling = true, origin = this.origin, direction = this.direction, normal$1 = tempVec3g) {
		const edge1 = tempVec3h;
		const edge2 = tempVec3i;
		const diff = tempVec3j;
		edge1.sub(b, a);
		edge2.sub(c, a);
		normal$1.cross(edge1, edge2);
		let DdN = direction.dot(normal$1);
		if (!DdN) return 0;
		let sign;
		if (DdN > 0) {
		...
		let QdN = -sign * diff.dot(normal$1);
		if (QdN < 0) return 0;
		return QdN / DdN;
	}
	getBarycoord(point$1, a, b, c, target = tempVec3h) {
		const v0 = tempVec3i;
		const v1 = tempVec3j;
		const v2 = tempVec3k;
		v0.sub(c, a);
...
*/
function rotateNormalBinormal(angle$1, norm, binorm, outNorm = norm, outBinorm = binorm) {
...
var Path = class {
	...
	*/
	computeFrenetFrames(divisions = 64, closed = false) {
		...
		const normal$1 = new Vec3();
		if (tx < ty && tx < tz) normal$1.set(1, 0, 0);
		else if (ty < tx && ty < tz) normal$1.set(0, 1, 0);
		else normal$1.set(0, 0, 1);
		const normals = new Array(divisions + 1);
		const binormals = new Array(divisions + 1);
		normals[0] = new Vec3();
		binormals[0] = new Vec3();
		tempVec3.cross(tangents[0], normal$1).normalize();
		...
		for (let i = 1; i < tangents.length; i++) {
			...
			if (crossLen > Number.EPSILON) {
				...
				mat4fromRotationSinCos(tempMat4$2, tempVec3, sinTheta, cosTheta);
		...
		if (closed === true) {
			...
			for (let i = 1; i < normals.length - 1; i++) {
				const angle$1 = step * i;
				rotateNormalBinormal(angle$1, normals[i], binormals[i]);
				tilts[i] += toDegrees(angle$1);
			}
...
var Post = class {
	...
	addPass({ vertex: vertex$8 = defaultVertex$3, fragment: fragment$7 = defaultFragment$3, uniforms = {}, textureUniform = "tMap", enabled = true } = {}) {
...
function Text({ font, text, width = Infinity, align = "left", size = 1, letterSpacing = 0, lineHeight = 1.4, wordSpacing = 0, wordBreak = false }) {
	const _this = this;
	let glyphs, buffers, baseline, scale$6;
	const newline = /\n/;
	const whitespace = /\s/;
	parseFont();
	createGeometry();
	...
	function layout() {
		...
		while (cursor < text.length && count < maxTimes) {
			...
			if (line.glyphs.length) {
				...
				let kern = getKernPairOffset(glyph.id, prevGlyph.id) * scale$6;
				line.width += kern;
				wordWidth += kern;
			}
			...
			advance += glyph.xadvance * scale$6;
			line.width += advance;
			wordWidth += advance;
			if (line.width > width) {
	...
	function populateBuffers(lines) {
		...
		for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
			...
			for (let i = 0; i < line.glyphs.length; i++) {
				...
				x += glyph.xoffset * scale$6;
				y -= glyph.yoffset * scale$6;
				let w = glyph.width * scale$6;
				let h = glyph.height * scale$6;
...
var GPGPU = class {
	...
	addPass({ vertex: vertex$8 = defaultVertex$2, fragment: fragment$7 = defaultFragment$2, uniforms = {}, textureUniform = "tMap", enabled = true } = {}) {
...
var Polyline = class {
	constructor(gl, { points, vertex: vertex$8 = defaultVertex$1, fragment: fragment$7 = defaultFragment$1, uniforms = {}, attributes = {} }) {
		this.gl = gl;
		this.points = points;
		...
		const uv$1 = new Float32Array(this.count * 2 * 2);
...
var Shadow = class {
	...
	add({ mesh, receive = true, cast = true, vertex: vertex$8 = defaultVertex, fragment: fragment$7 = defaultFragment, uniformProjection = "shadowProjectionMatrix", uniformView = "shadowViewMatrix", uniformTexture = "tShadow" }) {
		...
		if (vertex$8 === defaultVertex && fragment$7 === defaultFragment) {
...
function KhronosTextureContainer(buffer) {
	...
	const id$2 = new Uint8Array(buffer, 0, 12);
	for (let i = 0; i < id$2.length; i++) if (id$2[i] !== idCheck[i]) return console.error("File missing KTX identifier");
	...
	update(totalWeight = 1, isSet) {
		...
		this.data.forEach(({ node, transform, interpolation, times, values }) => {
			if (!this.duration) {
				...
				if (size$1 === 4) node[transform].slerp(val, weight);
...
var GLTFLoader = class {
	...
	static createTexture(gl, desc, images, { sampler: samplerIndex, source: sourceIndex, name, extensions, extras }) {
		...
		if (image.isBasis) {
			...
			const texture$1 = new Texture(gl, options);
			texture$1.name = name;
			image.texture = texture$1;
			return texture$1;
		}
	...
	static parseMeshes(gl, desc, bufferViews, materials, skins) {
		...
		return Promise.all(desc.meshes.map(async ({ primitives, weights, name, extensions, extras = {} }, meshIndex) => {
			...
			desc.nodes && desc.nodes.forEach(({ mesh, skin, extras: extras$1 }) => {
				if (mesh === meshIndex) {
					numInstances++;
					if (skin !== void 0) skinIndices.push(skin);
					if (extras$1 && extras$1.lightmap_scale_offset) isLightmap = true;
				}
			});
		...
		return Promise.all(primitives.map(async ({ attributes, indices, material: materialIndex, mode = 4, targets, extensions, extras }) => {
			...
			if (extensions && extensions.KHR_draco_mesh_compression) {
				...
				if (geometryData.index) {
					const data$1 = geometryData.index.array;
					const size = geometryData.index.itemSize;
					const buffer = gl.createBuffer();
					gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffer);
					gl.renderer.state.boundBuffer = buffer;
					gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, data$1, gl.STATIC_DRAW);
	...
	static parseNodes(gl, desc, meshes, skins, images) {
		...
		const cameras = [];
		const nodes = desc.nodes.map(({ camera, children, skin: skinIndex, matrix, mesh: meshIndex, rotation, scale: scale$6, translation, weights, name, extensions, extras }) => {
			...
			if (matrix) {
				...
				if (scale$6) node.scale.copy(scale$6);
		...
		meshes.forEach(({ primitives }, i) => {
			primitives.forEach((primitive, i$1) => {
...
var DracoManager = class {
	...
	onMessage({ data }) {
		const { id: id$2, error, geometry } = data;
		if (error) {
			console.log(error, id$2);
			return;
		}
		const geometryResolve = this.queue.get(id$2);
		this.queue.delete(id$2);
...
var BasisManager = class {
	...
	onMessage({ data }) {
		const { id: id$2, error, image } = data;
		if (error) {
			console.log(error, id$2);
			return;
		}
		const textureResolve = this.queue.get(id$2);
		this.queue.delete(id$2);
		image.isBasis = true;
